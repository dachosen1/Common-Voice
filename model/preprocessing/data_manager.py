...
def clean_data(filename: str, rate: int, signal: np.ndarray, label: object) -> None:
    document_path = r"C:\Users\ander\Documents"
    ...
    try:
        ...
        for row in range(flatten_data.shape[0]):
            train_test_choice = np.random.choice(
                ["train_data", "dev_data"], p=[0.8, 0.2]
            )
            save_path = os.path.join(document_path, train_test_choice, label)
...
def gender_split(wav_path: str, data_path: str, mel_seq_count: int = 648) -> None:
    ...
    for wav in tqdm(clip_wav):
        ...
        try:
            ...
            clean_data(wav_name, mel_seq_count, mel, label_name)
        except IndexError:
...
if __name__ == "__main__":
    gender_split(
        wav_path=r"C:\Users\ander\Documents\wav",
        data_path=r"C:\Users\ander\Google Drive\Projects\Common Voice\Development",
